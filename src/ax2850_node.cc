...
static double ENCODER_RESOLUTION = 1250*4;
double wheel_circumference = 78.5;
double wheel_base_length = 47.0;
double wheel_diameter = 25.0;
...
{
  // Node setup
  ros::init(argc, argv, "ax2850_node");
  ros::NodeHandle n;
  ...
  while(ros::ok()) 
  {
    ...
    if(!ros::ok())
      break;
    ROS_INFO("Will try to reconnect to the AX2850 in 5 seconds.");
    for (int i = 0; i < 100; ++i) 
	  {
